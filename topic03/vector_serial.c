...
int main(int argc, char **argv)
{
	...
	// populate the vector with non-trivial values
        for (int i = 0; i < num_arg; i++) {
                my_vector[i] = i; 
        }

	// start the internal benchmark timer
	clock_t start_time = clock();

	// sums the vector
	int my_sum = sum_vector(my_vector, num_arg);

	// stop the internal benchmark timer
	clock_t end_time = clock();

	// calculate the internal benchmark time
	double benchmark = (double)(end_time - start_time) / CLOCKS_PER_SEC;

	// prints the sum and the result of the internal benchmark
	printf("Sum: %d\n", my_sum);
